...
create view A as 
select distinct Filme.titulo, Filme.genero, Filme.avaliacao, Filme.classEtaria, count(Bilhete.idBilhete) as numTotalEspectadores, sum(Bilhete.preco) as LucroBilhetes, sum(PublicidadePatrocinador.valorPago) as LucroPublicidade 
...
select *, (A.LucroBilhetes + A.LucroPublicidade) as LucroTotal 
from A 
